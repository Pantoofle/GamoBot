...
async def start(ctx, *args):
    ...
    category = discord.utils.find(lambda cat: cat.name == table_name, ctx.guild.categories)
    if not category:
...
async def join(ctx, *args):
    ...
    chan = discord.utils.find(lambda chan: chan.name.lower() == name.lower(), ctx.guild.text_channels)
    if chan is not None:
        table = tables[chan]
        if table.open:
...
async def update_tables(guild):
    ...
    if index_msg is None:
        chan = discord.utils.find(lambda c: c.name == INDEX_CHAN, guild.channels)
